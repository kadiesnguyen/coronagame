...
class GameXucXacAdminController {
  ...
  setTiLeGame = catchAsync(async (req, res, next) => {
    ...
    await NhatKyHoatDong.insertNhatKyHoatDong({
      ...
      metadata: {
        tiLeBefore: result.gameConfigs.xucXacConfigs[`${this.CONFIG.KEY_SYSTEM_DB}`].tiLeCLTX,
        tiLeAfter: tiLe,
      },
    });
    return new OkResponse({
  ...
  setStatusAutoGame = catchAsync(async (req, res, next) => {
    ...
    await NhatKyHoatDong.insertNhatKyHoatDong({
      ...
      metadata: {
        statusBefore: result.gameConfigs.xucXacConfigs[`${this.CONFIG.KEY_SYSTEM_DB}`].autoGame,
  ...
  countAllGame = catchAsync(async (req, res, next) => {
    ...
    const countList = await this.CONFIG.MODEL.GAME_XUCXAC.countDocuments(query);
    return new OkResponse({
      data: countList,
    }).send(res);
  });
