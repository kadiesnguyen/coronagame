...
import { ADMIN_LIST_WITHDRAW_HISTORY_PAGE_SIZE, TINH_TRANG_WITHDRAW_HISTORY } from "@/configs/withdraw.config";
...
const WithdrawCard = ({ row, onOpen, onInfo }) => {
  const pending = row.tinhTrang === TINH_TRANG_WITHDRAW_HISTORY.DANG_CHO;
  return (
    <Box
      onClick={onOpen}
      sx={{
    ...
    >
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "8px", minWidth: 0 }}>
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: "8px", flexShrink: 0 }}>
          {convertJSXTinhTrangWithdrawHistory(row.tinhTrang)}
...
const ListWithdraw = ({ userId = "", statusGroup = "" }) => {
  const theme = useTheme();
  const isMobile = useMediaQuery(theme.breakpoints.down("md"));
  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(ADMIN_LIST_WITHDRAW_HISTORY_PAGE_SIZE);
  const { data: dataQuery, isLoading, refetch } = useGetListWithdrawHistory({
    page: page + 1,
    pageSize,
    userId,
    statusGroup,
  });
  const { data: rowCountState, refetch: refetchCount } = useGetCountAllWithdrawHistory({ userId, statusGroup });
  const isHistoryMode = statusGroup === "history";
  const sectionTitle =
    statusGroup === "pending" ? "Yêu cầu rút" : statusGroup === "history" ? "Lịch sử rút" : "Lịch sử rút tiền";
  const sectionSubtitle = isHistoryMode
    ? "Bấm dòng để xem chi tiết (chỉ xem)"
    : "Bấm dòng / icon mắt để duyệt tại chỗ";
  const emptyText = statusGroup === "pending" ? "Không có yêu cầu rút đang chờ" : "Không có lịch sử rút tiền";
  ...
  const isPending = selected?.tinhTrang === TINH_TRANG_WITHDRAW_HISTORY.DANG_CHO;
  ...
  const openDetail = (item) => {
    ...
    setBankForm({
      tenNganHang: item?.nganHang?.tenNganHang || "",
      tenChuTaiKhoan: item?.nganHang?.tenChuTaiKhoan || "",
      soTaiKhoan: item?.nganHang?.soTaiKhoan || "",
      bankCode: item?.nganHang?.bankCode || "",
    });
    setOpenDialog(true);
  };
      ...
      const results = await WithdrawService.editDetailedWithdrawHistory({
        ...
        noiDung:
          ...
          (tinhTrang === TINH_TRANG_WITHDRAW_HISTORY.HOAN_TAT
            ...
            : tinhTrang === TINH_TRANG_WITHDRAW_HISTORY.DA_HUY
              ? "Từ chối rút tiền"
              : ""),
        tinhTrang,
      });
        ...
        prev
          ? {
              ...
              nganHang: {
                ...
                tenChuTaiKhoan: bankForm.tenChuTaiKhoan,
                soTaiKhoan: bankForm.soTaiKhoan,
  ...
  const rows =
    dataQuery?.map((item, i) => ({
      ...
      nganHang: `${item.nganHang?.tenNganHang || ""} - ${item.nganHang?.soTaiKhoan || ""} - ${
        item.nganHang?.tenChuTaiKhoan || ""
      }`,
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 120,
      renderCell: (params) => convertJSXTinhTrangWithdrawHistory(params.row.tinhTrang),
      valueGetter: (params) => convertTinhTrangWithdrawHistory(params.row.tinhTrang),
    },
    { field: "createdAt", headerName: "Thời gian", width: 140 },
    {
      ...
      sortable: false,
      filterable: false,
      renderCell: (params) => <InfoButton onClick={() => openDetail(params.row.raw)} />,
    },
  ];
  ...
  return (
    <>
        ...
        {isMobile ? (
          <Box sx={{ display: "flex", flexDirection: "column", gap: "12px", width: "100%", minWidth: 0 }}>
            ...
            {rows.map((row) => (
              <WithdrawCard
                key={row.id}
                row={row}
                onOpen={() => openDetail(row.raw)}
                onInfo={() => openDetail(row.raw)}
              />
            ))}
            {pageCount > 1 && (
          ...
          <Box sx={{ ...adminDataGridBoxSx, height: 520 }}>
            <DataGrid
              ...
              onRowClick={(params) => openDetail(params.row.raw)}
              sx={{
                ...adminDataGridSx,
                cursor: "pointer",
      ...
      >
        ...
        >
          {isPending ? "Duyệt yêu cầu rút tiền" : "Chi tiết lịch sử rút"}
          ...
          {selected && (
            <>
              ...
              <Typography sx={{ fontSize: "1.3rem", color: "#b8c0d4" }}>
                ...
                {convertTinhTrangWithdrawHistory(selected.tinhTrang)}
              </Typography>
              ...
              {isPending ? (
                <Typography sx={{ fontSize: "1.2rem", color: "#ef6d6d" }}>
                  Từ chối = hoàn tiền cho user. Duyệt = xác nhận đã chuyển khoản.
                </Typography>
              ) : (
                <Typography sx={{ fontSize: "1.2rem", color: "#8b95a8" }}>
                  Đơn đã xử lý — chỉ xem, không đổi trạng thái.
                </Typography>
              )}
            </>
          )}
        ...
        >
          ...
          {isPending && (
            <>
              <Button
                onClick={() => handleUpdate(TINH_TRANG_WITHDRAW_HISTORY.DA_HUY)}
              ...
              <Button
                onClick={() => handleUpdate(TINH_TRANG_WITHDRAW_HISTORY.HOAN_TAT)}
      ...
      >
        ...
        <DialogActions sx={{ px: 3, pb: 2, gap: 1 }}>
          <Button variant="outlined" onClick={() => setOpenBankDialog(false)} sx={{ minHeight: 44 }}>
            Huỷ
          </Button>
          <Button
