...
import { convertLoaiCuoc, LOAI_CUOC_GAME } from "@/configs/game.xoso.config";
import useGetDetailedBetGameHistory from "@/hooks/admin/useGetDetailedBetGameHistory";

const transformDataChart = (dataQuery) => {
  const tempDataChart = Object.values(LOAI_CUOC_GAME).map((item) => {
    return {
      key: item,
      name: convertLoaiCuoc(item),
      value: 0,
    };
  });
  dataQuery?.map((item) => {
    const findItemCuoc = tempDataChart.find((itemDataChart) => itemDataChart.key === item.datCuoc[0].loaiCuoc);
    if (findItemCuoc) {
      findItemCuoc.value += item.datCuoc[0].tongTienCuoc;
    }
  });
