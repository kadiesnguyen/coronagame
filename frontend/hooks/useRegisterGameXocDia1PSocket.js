...
import { setKetQua, setKetQuaPhienTruoc, setPhien, setTimer, setTinhTrang } from "../redux/actions/gameXocDia1P";
  ...
  useEffect(() => {
    ...
    if (value.isConnected && value.socket) {
      if (isPlayGameXocDia1P) {
        ...
        value.socket.on(`${key_socket}:chuanBiQuay`, () => {
          dispatch(setTinhTrang(TINH_TRANG_GAME.CHUAN_BI_QUAY));
        });
