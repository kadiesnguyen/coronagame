...
import { setKetQua, setKetQuaPhienTruoc, setPhien, setTimer, setTinhTrang } from "../redux/actions/gameXucXac5P";

const useRegisterGameXucXac5PSocket = ({ value }) => {
  const dispatch = useDispatch();
  const router = useRouter();

  const { isPlayGame: isPlayGameXucXac5P } = useSelector((state) => state.gameXucXac5P);

  useEffect(() => {
    const key_socket = LOAI_GAME.XUCXAC5P;
    const onGamePage = router.pathname === `/games/${key_socket}`;
    if (value.isConnected && value.socket) {
      if (isPlayGameXucXac5P && onGamePage) {
  ...
  }, [value, isPlayGameXucXac5P, router.pathname]);
  return isPlayGameXucXac5P;
};
export default useRegisterGameXucXac5PSocket;
