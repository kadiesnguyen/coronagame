...
import { setKetQua, setKetQuaPhienTruoc, setPhien, setTimer, setTinhTrang } from "../redux/actions/gameXucXac10P";

const useRegisterGameXucXac10PSocket = ({ value }) => {
  const dispatch = useDispatch();
  const router = useRouter();

  const { isPlayGame: isPlayGameXucXac10P } = useSelector((state) => state.gameXucXac10P);

  useEffect(() => {
    const key_socket = LOAI_GAME.XUCXAC10P;
    const onGamePage = router.pathname === `/games/${key_socket}`;
    if (value.isConnected && value.socket) {
      if (isPlayGameXucXac10P && onGamePage) {
  ...
  }, [value, isPlayGameXucXac10P, router.pathname]);
  return isPlayGameXucXac10P;
};
export default useRegisterGameXucXac10PSocket;
